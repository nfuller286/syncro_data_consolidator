...
| `sdc_data_source_agent_override`| String    | Identifier for the agent/user SDC attributes this record to, if overriding source. | e.g., "Developer_SDC_System"                          |
...
| `primary_actor_user_id_authoritative`    | String    | Standardized/Authoritative User ID (e.g., the developer's Syncro ID).                |                                                     |
...
*Purpose: For sub-items like Syncro ticket comments, `notes.json` sub-notes/todos, or individual messages within an ST session if not placed in `source_specific_details`.*
